                        ...
                        @foreach ($cat->childs as $subcat)
                            <li @if ( branchIsActive($subcat->childs, $subcat->slug) )class="active"@endif>
                                ...
                                @if (count($subcat->childs) > 0)
                                    <ul class="list-unstyled" @if ( branchIsActive($subcat->childs, $subcat->slug) )style="display: block;"@endif>
                                        @foreach ($subcat->childs as $child)
                                            <li @if ( branchIsActive($child->childs, $child->slug) )class="active"@endif>
                                                ...
                                                @if (count($child->childs) > 0)
                                                    <ul class="list-unstyled" @if ( branchIsActive($child->childs, $child->slug) )style="display: block;"@endif>
                                                        @foreach ($child->childs as $child2)
                                                            <li @if ( branchIsActive([], $child2->slug) )class="active"@endif><a class="item-name" href="{{ URL::to($child2->slug) }}">{{ $child2->name }}</a></li>
                                                        @endforeach
                                                    </ul>
                                                @endif
                                            </li>
